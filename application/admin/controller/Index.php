...
class Index extends Base
{
    ...
    public function front()
    {
        ...
        $html = "";
        foreach ($class as $v) {
            $html .= ' <form class="cgwl-item" action="' . $action . '/index/index/home?visiter_id=&visiter_name=&avatar=&business_id=' . $login['business_id'] . '&groupid=' . $v['id'] . '" method="post" target="_blank" >';
            $html .= ' <input type="hidden" name="product" value="">';
            $html .= ' <input type="submit" value="' . $v['groupname'] . '"></form>';
        }


        $str = '<link rel="stylesheet" href="' . $web . '__style__/index/cgwl_online.css">';
        $str .= '<div class="cgwl-form"  id="cgwl-kefu"><i class="cgwl-icon"></i> ';
        $str .= '<form class="cgwl-item" action="' . $action . '/index/index/home?visiter_id=&visiter_name=&avatar=&business_id=' . $login['business_id'] . '&groupid=0" method="post" target="_blank" >';
